...
function getAllFlavors() { return [...DEFAULT_FLAVORS, ...getCustomFlavors()]; }
...
function getActiveColor() { return sessionStorage.getItem(KEY_ACTIVE_COLOR) || "#ff2d2d"; }
function getActiveName()  { return sessionStorage.getItem(KEY_ACTIVE_NAME)  || null; }
...
function drawWheel() {
  ...
  for (let i = 0; i < numSegments; i++) {
    ...
    ctx.fillStyle = flavors[i].color; ctx.fill();

    const g = ctx.createRadialGradient(CX,CY,R*0.1,CX,CY,R);
    g.addColorStop(0,"rgba(0,0,0,0.3)"); g.addColorStop(0.6,"rgba(0,0,0,0)"); g.addColorStop(1,"rgba(255,255,255,0.06)");
    ctx.fillStyle = g; ctx.fill();

    ctx.beginPath(); ctx.moveTo(CX,CY); ctx.arc(CX,CY,R,s,e); ctx.closePath();
    ctx.strokeStyle = "#161918"; ctx.lineWidth = 2; ctx.stroke();

    ctx.save(); ctx.translate(CX,CY); ctx.rotate(s+arc/2); ctx.textAlign="right";
    ctx.font='600 11.5px "IBM Plex Mono",monospace';
    ctx.fillStyle="rgba(255,255,255,0.95)"; ctx.shadowColor="rgba(0,0,0,0.9)"; ctx.shadowBlur=6;
    ctx.fillText(flavors[i].name, R-16, 4.5); ctx.restore();
  }
  ...
  for (let i=0;i<tt;i++) {
    const a=currentAngle+(i/tt)*2*Math.PI, m=i%4===0, l=m?11:5;
    ctx.beginPath();
    ...
    ctx.lineWidth=m?2:1; ctx.stroke();
  }
  ctx.beginPath(); ctx.arc(CX,CY,22,0,2*Math.PI);
  ctx.fillStyle="#161918"; ctx.fill();
...
function spin() {
  ...
  let tickAccum=0, prev=currentAngle;
  ...
  function animate(now) {
    ...
    isSpinning=false; btn.classList.remove("spinning");
    const winner = flavors[getWinningIndex()];
    currentWinner = winner;
    spinCount++;

    const ctr = document.getElementById("spinCounter");
    ctr.textContent=`×${spinCount}`; ctr.classList.remove("hidden");
    btn.textContent="SPIN AGAIN"; btn.disabled=false;
    ...
    updateHistoryBadge(); updateStreak(); updateStats();
    showPopup(winner);
  }
...
function applyThemeFull(color) {
  ...
  if (wrap) wrap.style.boxShadow=`0 0 0 2px rgba(255,255,255,0.06),0 0 80px rgba(${r},${g},${b},0.45),0 20px 100px rgba(0,0,0,0.8)`;
  ...
  if (bw) { bw.style.color=color; bw.style.textShadow=`0 0 28px rgba(${r},${g},${b},0.5)`; }

  // Header brand
  const hb = document.querySelector("header .brand");
  if (hb) { hb.style.color=color; hb.style.textShadow=`0 0 24px rgba(${r},${g},${b},0.55)`; }
...
function applyIconBtnColor(color, r, g, b) {
  ...
  if (!sheet) { sheet=document.createElement("style"); sheet.id="_iconBtnSheet"; document.head.appendChild(sheet); }
  sheet.textContent = `
    .icon-btn {
...
class CP {
  constructor(x,y,color) {
    Object.assign(this,{x,y,color,vx:(Math.random()-.5)*14,vy:-(Math.random()*16+6),
      gravity:0.55,rotation:Math.random()*360,rotSpeed:(Math.random()-.5)*12,
      w:Math.random()*10+5,h:Math.random()*5+3,life:1,decay:Math.random()*0.014+0.008,
      shape:Math.random()<0.4?"circle":"rect"});
  }
  update() {
    this.x+=this.vx; this.y+=this.vy; this.vy+=this.gravity;
    this.vx*=0.99; this.rotation+=this.rotSpeed; this.life-=this.decay;
  }
  draw(c) {
    c.save(); c.globalAlpha=Math.max(this.life,0);
    c.translate(this.x,this.y); c.rotate(this.rotation*Math.PI/180);
    c.fillStyle=this.color;
    if(this.shape==="circle"){c.beginPath();c.arc(0,0,this.w/2,0,2*Math.PI);c.fill();}
...
function launchConfetti(wc) {
  ...
  const pal=[wc,`rgba(255,255,255,0.9)`,`rgba(${r},${g},${b},0.65)`,
    `rgba(${Math.min(r+70,255)},${Math.min(g+70,255)},${Math.min(b+70,255)},0.85)`,"#fff"];
  for(let i=0;i<160;i++) particles.push(new CP(cx,cy,pal[Math.floor(Math.random()*pal.length)]));
...
function animConfetti() {
  ...
  if(particles.length>0){cRAF=requestAnimationFrame(animConfetti);}
  else{cRAF=null; cctx.clearRect(0,0,confettiCanvas.width,confettiCanvas.height);}
}

// ── Flavor of the Day ─────────────────────────────────────────
// Seed: full date (YYYY + dayOfYear) cycles through ALL flavors including custom
function getFOTD() {
  const all=getAllFlavors(), now=new Date();
...
// ── Streak ────────────────────────────────────────────────────
function getStreakCount() { try{return parseInt(localStorage.getItem(KEY_STREAK)||"0");}catch{return 0;} }
function updateStreak()   { localStorage.setItem(KEY_STREAK, getStreakCount()+1); }

// ── Passport ─────────────────────────────────────────────────
function getPassport() { try{return JSON.parse(localStorage.getItem(KEY_PASSPORT)||"[]");}catch{return[];} }
function setPassport(d){ localStorage.setItem(KEY_PASSPORT,JSON.stringify(d)); }

function saveSpinToPassport(f) {
  const log=getPassport();
  log.unshift({id:Date.now(),name:f.name,color:f.color,ts:new Date().toISOString()});
  if(log.length>100)log.pop(); setPassport(log); renderPassport();
}

function renderPassport() {
  const log=getPassport(), list=document.getElementById("passportList");
  if(!list) return;
  if(!log.length){list.innerHTML=`<div class="empty-state"><span class="empty-icon">🌀</span><p>No spins yet.<br/>Give the wheel a go.</p></div>`;return;}
...
function clearPassport(){
  ...
  setPassport([]); renderPassport(); updateHistoryBadge(); updateStats();
}

// ── Orders ────────────────────────────────────────────────────
function getOrders() { try{return JSON.parse(localStorage.getItem(KEY_ORDERS)||"[]");}catch{return[];} }
function setOrders(d){ localStorage.setItem(KEY_ORDERS,JSON.stringify(d)); }

function saveOrder(f,qty) {
  const o=getOrders();
  o.unshift({id:Date.now(),name:f.name,color:f.color,qty,ts:new Date().toISOString()});
  if(o.length>100)o.pop(); setOrders(o); renderOrders(); updateOrdersBadge(); updateStats();
}

function renderOrders() {
  const o=getOrders(), list=document.getElementById("orderList");
  if(!list) return;
  if(!o.length){list.innerHTML=`<div class="empty-state"><span class="empty-icon">🧃</span><p>No orders yet.<br/>Spin and place one.</p></div>`;return;}
  list.innerHTML=o.map(r=>`
    <div class="history-entry">
...
function clearOrders(){
  ...
  setOrders([]); renderOrders(); updateOrdersBadge(); updateStats();
}

// ── Badges ────────────────────────────────────────────────────
function updateHistoryBadge(){
  const b=document.getElementById("historyBadge"); if(!b)return;
  const n=getPassport().length; b.textContent=n>99?"99+":n; b.classList.toggle("hidden",n===0);
}
function updateOrdersBadge(){
  const b=document.getElementById("ordersBadge"); if(!b)return;
  const n=getOrders().length; b.textContent=n>99?"99+":n; b.classList.toggle("hidden",n===0);
}
...
function populateMixerSelects(){
  ...
  const opts=all.map((f,i)=>`<option value="${i}">${f.name}</option>`).join("");
  const a=document.getElementById("mixerA"), b=document.getElementById("mixerB");
  if(!a||!b)return; a.innerHTML=opts; b.innerHTML=opts; a.value="0"; b.value="1"; syncDots();
}

function syncDots(){
  const all=getAllFlavors();
  const a=document.getElementById("mixerA"), b=document.getElementById("mixerB");
  const dA=document.getElementById("dotA"),  dB=document.getElementById("dotB");
  if(!a||!b||!dA||!dB)return;
  const fa=all[parseInt(a.value)], fb=all[parseInt(b.value)];
  if(fa){dA.style.background=fa.color;dA.style.boxShadow=`0 0 8px ${fa.color}`;}
  if(fb){dB.style.background=fb.color;dB.style.boxShadow=`0 0 8px ${fb.color}`;}
}

function blendColors(hA,hB){
  const a=hexToRgb(hA), b=hexToRgb(hB);
  return "#"+[Math.round((a.r+b.r)/2),Math.round((a.g+b.g)/2),Math.round((a.b+b.b)/2)].map(v=>v.toString(16).padStart(2,"0")).join("");
}
function blendNames(nA,nB){
  const a=nA.replace(/\s+/g,""), b=nB.replace(/\s+/g,"");
  const raw=a.slice(0,Math.ceil(a.length/2))+b.slice(Math.floor(b.length/2));
  return raw.charAt(0).toUpperCase()+raw.slice(1).toLowerCase();
}

let blendedWinner=null, mixQty=1;
...
function runQuantumMix(){
  ...
  const oA=document.getElementById("orbA"), oB=document.getElementById("orbB");
  oA.style.background=fa.color; oA.style.boxShadow=`0 0 20px ${fa.color}`;
  oB.style.background=fb.color; oB.style.boxShadow=`0 0 20px ${fb.color}`;
  document.getElementById("mixResult").classList.add("hidden");
  document.getElementById("mixLoading").classList.remove("hidden");

  setTimeout(()=>{
    const bc=blendColors(fa.color,fb.color), bn=blendNames(fa.name,fb.name);
    blendedWinner={name:bn, color:bc}; mixQty=1;
    ...
    sb.disabled=false; sb.dataset.name=bn; sb.dataset.color=bc; sb.textContent="Save to Lab ✦";
    ...
    if(os){
      ...
      if(ob){ob.style.background=bc; ob.style.boxShadow=`0 4px 28px ${bc}55`;}
      const qd=document.getElementById("mixQtyDisplay"); if(qd)qd.textContent="1";
    }
...
function updateMixQty(delta){
  ...
  const el=document.getElementById("mixQtyDisplay"); if(el)el.textContent=mixQty;
}
...
function saveBlendToLab(){
  ...
  const name=btn.dataset.name, color=btn.dataset.color;
  if(!name||!color)return;
  const custom=getCustomFlavors();
  if(custom.some(f=>f.name===name)){btn.textContent="Already saved ✓";btn.disabled=true;return;}
  custom.push({name,color,default:false});
  localStorage.setItem(KEY_CUSTOM,JSON.stringify(custom));
  reloadFlavors(); populateMixerSelects(); updateStats();
  btn.textContent="Saved to Lab ✓"; btn.disabled=true;
}
...
function showPopup(flavor){
  ...
  fe.textContent=flavor.name; fe.style.color=flavor.color;
  fe.style.textShadow=`0 0 40px ${flavor.color}55`;

  const ob=document.getElementById("orderBtn");
  if(ob){ob.style.background=flavor.color; ob.style.boxShadow=`0 4px 28px ${flavor.color}55`;}
...
function updateQuantity(delta){
  ...
  const el=document.getElementById("qtyDisplay"); if(el)el.textContent=quantity;
}
...
function reloadFlavors(){
  flavors=getAllFlavors(); numSegments=flavors.length; arc=(2*Math.PI)/numSegments; drawWheel();
}
...
document.addEventListener("keydown",e=>{
  ...
  if(e.code==="Escape"){closePopup();closeDrawer();closeQuantum();}
...
const mxA=document.getElementById("mixerA"), mxB=document.getElementById("mixerB");
